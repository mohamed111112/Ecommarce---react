  ...
  return (
    <section className={`container ${styles.categories} `}>
      ...
      <div>
          ...
          {categories?.map((category) => {
            ...
            return (
              <div className="row g-3 m-0" key={category._id}>
                <div
                  className={`border rounded-2 overflow-hidden p-0 d-flex ${styles.categoryHover} flex-column flex-md-row`}
                >
                  <div
                    className={`${
                      hasSubCategories ? "col-12 col-md-6" : "col-12"
                    } category mb-0`}
                  >
                    <Link
                    ...
                    >
                      {subCategories
                        ...
                        ?.map((subCat) => (
                          <li key={subCat._id}>
                            <Link
                              to={`${subCat.slug}?type=subCat&id=${subCat._id}`}
                            >
                              {subCat.name}
                            </Link>
                          </li>
        ...
        {error && !categories && <p className="text-center text-danger my-5"> Couldn't get the categories, please refresh the page.</p>}
      </div>
    </section>
  );
}
